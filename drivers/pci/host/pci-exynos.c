...
static struct pci_dev *exynos_pcie_get_pci_dev(struct pcie_port *pp)
{
	...
	dev_dbg(pp->dev, "%s : device_id & vendor_id = 0x%08x\n", __func__, val);
...
static int exynos_pcie_set_l1ss(int enable, struct pcie_port *pp, int id)
{
	...
	if (exynos_pcie->ep_device_type != EP_BCM_WIFI) {
		dev_dbg(pp->dev, "Can't set L1SS(It's not WIFI device)!!!\n");
		return -EINVAL;
	}
	...
	if (exynos_pcie->state != STATE_LINK_UP || exynos_pcie->atu_ok == 0) {
		...
		dev_dbg(pp->dev, "%s: It's not needed. This will be set later."
				"(state = 0x%x, id = 0x%x)\n",
	...
	if (ep_pci_dev == NULL) {
		spin_unlock_irqrestore(&exynos_pcie->conf_lock, flags);
		dev_dbg(pp->dev, "Failed to set L1SS %s (pci_dev == NULL)!!!\n",
					enable ? "ENABLE" : "FALSE");
		return -EINVAL;
	}
		...
		const char *buf, size_t count)
{
	...
	switch (op_num) {
	case 0:
		dev_dbg(dev, "PCIe UNIT test START.\n");

		if (wlan_gpio >= 0) {
			gpio_set_value(wlan_gpio, 1);
		} else {
			dev_dbg(dev, "WLAN_RGT_ON is not defined!!!\n");
			test_result = -1;
			break;
		}
		...
		dev_dbg(dev, "PCIe UNIT test SUCCESS!!!\n");
	...
	case 4:
		dev_dbg(dev, "L1.2 En/Disable thread Stop!!!\n");
		if (task != NULL)
			kthread_stop(task);
		else
			dev_dbg(dev, "task is NULL!!!!\n");
		break;

	case 5:
		dev_dbg(dev, "Enable SysMMU feature.\n");
		exynos_pcie->use_sysmmu = true;
		pcie_sysmmu_enable();

		break;
	case 6:
		dev_dbg(dev, "Disable SysMMU feature.\n");
		exynos_pcie->use_sysmmu = false;
		pcie_sysmmu_disable();
		break;

	case 7:
		dev_dbg(dev, "SysMMU MAP - 0x%llx(sz:0x%llx)\n",
				iommu_addr, iommu_size);
		pcie_iommu_map(iommu_addr, iommu_addr, iommu_size, 0x3);
		break;

	case 8:
		dev_dbg(dev, "SysMMU UnMAP - 0x%llx(sz:0x%llx)\n",
			iommu_addr, iommu_size);
	...
	case 9:
		do {
			...
			dev_dbg(dev, "PCIe hard access Start.(20min)\n");
			/* DBI access */
			if (chk_hard_access(exynos_pcie->rc_dbi_base)) {
				dev_dbg(dev, "DBI hared access test FAIL!\n");
				break;
			}

			/* EP configuration access */
			if (chk_hard_access(pp->va_cfg0_base)) {
				dev_dbg(dev, "EP config access test FAIL!\n");
				break;
			}
			...
			if (chk_hard_access(ep_mem_region)) {
				dev_dbg(dev, "EP mem access test FAIL!\n");
				iounmap(ep_mem_region);
				break;
			}
			iounmap(ep_mem_region);
			dev_dbg(dev, "PCIe hard access Success.\n");
		} while (0);

		break;

	case 10:
		dev_dbg(dev, "L1.2 Disable....\n");
		exynos_pcie_l1ss_ctrl(0, 0x40000000);
		break;
	case 11:
		dev_dbg(dev, "L1.2 Enable....\n");
		exynos_pcie_l1ss_ctrl(1, 0x40000000);
		break;

	default:
		dev_dbg(dev, "Unsupported Test Number(%d)...\n", op_num);
	}
							...
							int event)
{
	...
	if (exynos_pcie->event_reg && exynos_pcie->event_reg->callback &&
		...
		dev_dbg(pp->dev, "Callback for the event : %d\n", event);
		exynos_pcie->event_reg->callback(notify);
	} else {
		dev_dbg(pp->dev, "Client driver does not have registration "
					"of the event : %d\n", event);
		dev_dbg(pp->dev, "Force PCIe poweroff --> poweron\n");
		exynos_pcie_poweroff(exynos_pcie->ch_num);
		exynos_pcie_poweron(exynos_pcie->ch_num);
	}
}
	...
	for (i = 0; i < total_clk_num; i++) {
		if (i < exynos_pcie->pcie_clk_num) {
			...
			if (IS_ERR(clks->pcie_clks[i])) {
				dev_dbg(pp->dev, "Failed to get pcie clock\n");
				return -ENODEV;
			}
		} else {
...
retry:
	...
	if (count >= MAX_TIMEOUT) {
		try_cnt++;
		dev_dbg(dev, "%s: Link is not up, try count: %d, %x\n",
			__func__, try_cnt,
			exynos_elb_readl(exynos_pcie, PCIE_ELBI_RDLH_LINKUP));
		if (try_cnt < 10) {
			gpio_set_value(exynos_pcie->perst_gpio, 0);
...
void exynos_pcie_dislink_work(struct work_struct *work)
{
	...
	dev_dbg(dev, "link down and recovery cnt: %d\n",
			exynos_pcie->linkdown_cnt);
...
void exynos_pcie_work_l1ss(struct work_struct *work)
{
	...
	if (exynos_pcie->work_l1ss_cnt == 0) {
		dev_dbg(dev, "[%s]boot_cnt: %d, work_l1ss_cnt: %d\n",
...
static void exynos_pcie_assert_phy_reset(struct pcie_port *pp)
{
	...
	dev_dbg(pp->dev, "phy clk enable, ret value = %d\n", ret);
	...
	if (exynos_pcie->use_ia) {
		...
		dev_dbg(pp->dev, "Set I/A for CDR reset.");
...
static irqreturn_t exynos_pcie_irq_handler(int irq, void *arg)
{
	...
	if (val & IRQ_LINK_DOWN) {
		dev_dbg(pp->dev, "!!!PCIE LINK DOWN (state : 0x%x)!!!\n", val);
				...
				u32 *val)
{
	...
	ret = regmap_read(exynos_pcie->pmureg,
			...
			&reg_val);
	if (reg_val == 0 || ret != 0) {
		dev_dbg(pp->dev, "PCIe PHY is disabled...\n");
		*val = 0xffffffff;
				...
				struct platform_device *pdev)
{
	...
	if (!pp->irq) {
		dev_dbg(&pdev->dev, "failed to get irq\n");
		return -ENODEV;
	}
...
#endif
	if (ret) {
		dev_dbg(&pdev->dev, "failed to initialize host\n");
		return ret;
	}
...
static void enable_cache_cohernecy(struct pcie_port *pp, int enable)
{
	...
	if (exynos_pcie->ip_ver > 0x889500) {
		/* Set enable value */
		if (enable) {
			dev_dbg(pp->dev, "Enable cache coherency.\n");
			set_val = PCIE_SYSREG_SHARABLE_ENABLE;
		} else {
			dev_dbg(pp->dev, "Disable cache coherency.\n");
...
static int exynos_pcie_parse_dt(struct device_node *np, struct pcie_port *pp)
{
	...
	if (of_property_read_u32(np, "ip-ver", &exynos_pcie->ip_ver)) {
		dev_dbg(pp->dev, "Failed to parse the number of ip-ver\n");
		return -EINVAL;
	}

	if (of_property_read_u32(np, "pcie-clk-num",
					&exynos_pcie->pcie_clk_num)) {
		dev_dbg(pp->dev, "Failed to parse the number of pcie clock\n");
		return -EINVAL;
	}

	if (of_property_read_u32(np, "phy-clk-num",
					&exynos_pcie->phy_clk_num)) {
		dev_dbg(pp->dev, "Failed to parse the number of phy clock\n");
		return -EINVAL;
	}

	if (of_property_read_u32(np, "ep-device-type",
				&exynos_pcie->ep_device_type)) {
		dev_dbg(pp->dev, "EP device type is NOT defined...(WIFI?)\n");
	...
	if (of_property_read_u32(np, "max-link-speed",
		...
		dev_dbg(pp->dev, "MAX Link Speed is NOT defined...(GEN1)\n");
	...
	if (!of_property_read_string(np, "use-msi", &use_msi)) {
		if (!strcmp(use_msi, "true")) {
			...
			dev_dbg(pp->dev, "Invalid use-msi value"
					"(Set to default -> true)\n");
			exynos_pcie->use_msi = true;
		}
	} else {
	...
	if (!of_property_read_string(np, "use-sicd", &use_sicd)) {
		if (!strcmp(use_sicd, "true")) {
			...
			dev_dbg(pp->dev, "Invalid use-sicd value"
				       "(set to default -> false)\n");
			exynos_pcie->use_sicd = false;
		}
	} else {
	...
	if (!of_property_read_string(np, "use-sysmmu", &use_sysmmu)) {
		if (!strcmp(use_sysmmu, "true")) {
			dev_dbg(pp->dev, "PCIe SysMMU is ENABLED.\n");
			exynos_pcie->use_sysmmu = true;
		} else if (!strcmp(use_sysmmu, "false")) {
			dev_dbg(pp->dev, "PCIe SysMMU is DISABLED.\n");
			exynos_pcie->use_sysmmu = false;
		} else {
			dev_dbg(pp->dev, "Invalid use-sysmmu value"
				       "(set to default -> false)\n");
			exynos_pcie->use_sysmmu = false;
		}
	} else {
	...
	if (!of_property_read_string(np, "use-ia", &use_ia)) {
		if (!strcmp(use_ia, "true")) {
			dev_dbg(pp->dev, "PCIe I/A is ENABLED.\n");
			exynos_pcie->use_ia = true;
		} else if (!strcmp(use_ia, "false")) {
			dev_dbg(pp->dev, "PCIe I/A is DISABLED.\n");
			exynos_pcie->use_ia = false;
		} else {
			dev_dbg(pp->dev, "Invalid use-ia value"
				       "(set to default -> false)\n");
			exynos_pcie->use_ia = false;
		}
	} else {
	...
	if (IS_ERR(exynos_pcie->pmureg)) {
		dev_dbg(pp->dev, "syscon regmap lookup failed.\n");
	...
	if (IS_ERR(exynos_pcie->sysreg) && IS_ERR(exynos_pcie->sysreg_base)) {
		dev_dbg(pp->dev, "SYSREG is not defined.\n");
		return PTR_ERR(exynos_pcie->sysreg);
	}

	wlan_gpio = of_get_named_gpio(np, "pcie,wlan-gpio", 0);
	if (wlan_gpio >= 0) {
		dev_dbg(pp->dev, "WLAN_RGT_ON is defined for test.\n");
...
static int __init exynos_pcie_probe(struct platform_device *pdev)
{
	...
	if (create_pcie_sys_file(&pdev->dev))
		dev_dbg(&pdev->dev, "Failed to create pcie sys file\n");

	if (of_property_read_u32(np, "ch-num", &ch_num)) {
		dev_dbg(&pdev->dev, "Failed to parse the channel number\n");
		return -EINVAL;
	}
	...
	if (exynos_pcie->perst_gpio < 0) {
		dev_dbg(&pdev->dev, "cannot get perst_gpio\n");
	} else {
	...
	if (IS_ERR(exynos_pcie->pcie_pinctrl)) {
		dev_dbg(&pdev->dev, "Can't get pcie pinctrl!!!\n");
		goto probe_fail;
	}
	...
	if (IS_ERR(exynos_pcie->pin_state[PCIE_PIN_DEFAULT])) {
		dev_dbg(&pdev->dev, "Can't set pcie clkerq to output high!\n");
		goto probe_fail;
	}
	...
	if (IS_ERR(exynos_pcie->pin_state[PCIE_PIN_IDLE]))
		dev_dbg(&pdev->dev, "No idle pin state(but it's OK)!!\n");
	else
	...
	if (exynos_pcie->idle_ip_index < 0) {
		dev_dbg(&pdev->dev, "Cant get idle_ip_dex!!!\n");
		ret = -EINVAL;
		goto probe_fail;
	}
	...
	if (ret) {
		dev_dbg(&pdev->dev, "Failed to register lpa notifier\n");
		goto probe_fail;
	}
#endif
	...
	if (IS_ERR(exynos_pcie->pcie_wq)) {
		dev_dbg(pp->dev, "couldn't create workqueue\n");
		ret = EBUSY;
		goto probe_fail;
	}
	...
	if (exynos_pcie->ch_num == 0) {
		...
		if (ret < 0) {
			dev_dbg(&pdev->dev, "Failed to register WIFI notifier\n");
			goto probe_fail;
		}

		ret = sec_argos_register_notifier(&argos_l1ss_nb2, "P2P");
		if (ret < 0) {
			dev_dbg(&pdev->dev, "Failed to register P2P notifier\n");
			goto probe_fail;
		}
	}
...
probe_fail:
	if (ret)
		dev_dbg(&pdev->dev, "%s: pcie probe failed\n", __func__);
	else
		dev_dbg(&pdev->dev, "%s: pcie probe success\n", __func__);
...
static void exynos_pcie_resumed_phydown(struct pcie_port *pp)
{
	...
	dev_dbg(pp->dev, "pcie clk enable, ret value = %d\n", ret);
...
void exynos_pcie_config_l1ss(struct pcie_port *pp)
{
	...
	*/
	...
	if (exynos_pcie->l1ss_ctrl_id_state == 0) {
		...
		dev_dbg(pp->dev, "l1ss enabled(0x%x)\n",
		...
		exynos_pcie_wr_own_conf(pp, exp_cap_off + PCI_EXP_LNKCTL,
						4, val);
		dev_dbg(pp->dev, "l1ss disabled(0x%x)\n",
				exynos_pcie->l1ss_ctrl_id_state);
	}
	spin_unlock_irqrestore(&exynos_pcie->conf_lock, flags);
...
int exynos_pcie_poweron(int ch_num)
{
	...
	if (exynos_pcie->state == STATE_LINK_DOWN) {
		...
		if (exynos_pcie_establish_link(pp)) {
			dev_dbg(pp->dev, "pcie link up fail\n");
			goto poweron_fail;
		}
#ifdef NCLK_OFF_CONTROL
		if (exynos_pcie->ip_ver == 0x981000)
		...
		if (!exynos_pcie->probe_ok) {
			...
			if (!exynos_pcie->pci_dev) {
				dev_dbg(pp->dev, "Failed to get pci device\n");
				goto poweron_fail;
			}
			...
			if (pci_save_state(exynos_pcie->pci_dev)) {
				dev_dbg(pp->dev, "Failed to save pcie state\n");
				goto poweron_fail;
			}
			...
			if (pci_load_saved_state(exynos_pcie->pci_dev,
					     exynos_pcie->pci_saved_configs)) {
				dev_dbg(pp->dev, "Failed to load pcie state\n");
				goto poweron_fail;
			}
...
void exynos_pcie_poweroff(int ch_num)
{
	...
	if (exynos_pcie->state == STATE_LINK_UP ||
		...
		gpio_set_value(exynos_pcie->perst_gpio, 0);
		dev_dbg(pp->dev, "%s: Set PERST to LOW, gpio val = %d\n",
			__func__, gpio_get_value(exynos_pcie->perst_gpio));
...
retry_pme_turnoff:
	val = readl(exynos_pcie->elbi_base + PCIE_ELBI_RDLH_LINKUP) & 0x1f;
	dev_dbg(dev, "Current LTSSM State is 0x%x with retry_cnt =%d.\n",
								val, retry_cnt);
	...
	if (count >= MAX_TIMEOUT)
		dev_dbg(dev, "cannot receive ack message from wifi\n");
	...
	if (count >= MAX_TIMEOUT) {
		...
		dev_dbg(dev, "cannot receive L23_READY DLLP packet(0x%x)\n", val);
#ifdef CONFIG_SEC_PANIC_PCIE_ERR
...
void exynos_pcie_pm_suspend(int ch_num)
{
	...
	if (exynos_pcie->state == STATE_LINK_DOWN) {
		dev_dbg(pp->dev, "RC%d already off\n", exynos_pcie->ch_num);
		return;
	}
...
int exynos_pcie_deregister_event(struct exynos_pcie_register_event *reg)
{
	...
	if (pp) {
		...
		dev_dbg(pp->dev, "Event is deregistered for RC %d\n",
				exynos_pcie->ch_num);
	} else {
		pr_err("PCIe: did not find RC for pci endpoint device\n");
		ret = -ENODEV;
